namespace FeedTime.Pages
{
    ...
    public class CreateSleepPageBase : Page
    {
        private readonly NavigationHelper navigationHelper;
        private readonly ResourceLoader resourceLoader = ResourceLoader.GetForCurrentView();
        private readonly SleepActivityViewModel viewModel;
        ...
        /// </summary>
        public SleepActivityViewModel DefaultViewModel
        {
        ...
        protected async void NavigationHelper_LoadState(object sender, LoadStateEventArgs e)
        {
            ...
            var navigationParameter = e.NavigationParameter as ViewModelNavigationParameter<SleepActivityViewModel>;

            bool createNewSleep = string.IsNullOrWhiteSpace(DefaultViewModel.Id);
            ...
            if (createNewSleep)
            {
                // Start the sleep and store it in Azure so it can be accessed on another device
                var createdActivity = await DataSourceFactory.Current
                                                             .CreateSleepActivity(viewModel.AsModel());
                DefaultViewModel.Id = createdActivity.Id;
                settings.InProgressSleepActivityId = createdActivity.Id;
                ...
                // We've just created a new sleep which invalidates any scheduled sleep 
                // notifications so delete them
                if (!string.IsNullOrWhiteSpace(settings.NextSleepNotificationId))
                {
                    var toastNotifier = ToastNotificationManager.CreateToastNotifier();
                    var scheduledNotificaiton = toastNotifier.GetScheduledToastNotifications()
                                                             .SingleOrDefault(tn => tn.Id == settings.NextSleepNotificationId);
