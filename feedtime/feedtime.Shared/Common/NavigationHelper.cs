...
namespace FeedTime.Common
{
    ...
    public class NavigationHelper : DependencyObject
    {
        ...
        public static Type DeterminePageFromNavigationArguments(string navigationArguments)
        {
            //var parsedArguments = Regex.Match(navigationArguments,
            //                                  @"(?<arguments>([a-zA-Z0-9]*)=([a-zA-Z0-9]*)&?)*");

            //var group = parsedArguments.Groups["arguments"];

            switch (navigationArguments)
            {
                case "activity=Feed":
                    return typeof(CreateFeedPage);
                default:
                    return typeof(MainPage);
            }
        }

        public static async Task<Type> DetermineFirstPage()
        {
            ...
            bool couldLogIn = await currentDataSource.Login(false, true);
            UserProfile currentUserProfile = null;
            if (couldLogIn)
            {
                currentSettings.MobileServicesUserId = currentDataSource.CurrentUserId;
                currentUserProfile = await currentDataSource.RetrieveUserProfile();
            }

            if (currentUserProfile == null)
                return typeof(SplashPage);

            // If the user does not have a family then create one for them
            if (string.IsNullOrWhiteSpace(currentUserProfile.FamilyId))
            {
                var family = await currentDataSource.CreateFamily(new Family());
                currentUserProfile.FamilyId = family.Id;
                await currentDataSource.UpdateUserProfile(currentUserProfile);
            }

            if (currentSettings.FamilyId != currentUserProfile.FamilyId)
                currentSettings.MostRecentlyAccessedBabyId = null;
            currentSettings.FamilyId = currentUserProfile.FamilyId;

            // If the user has a 'last used' baby then continue to the main page
            if (string.IsNullOrWhiteSpace(currentSettings.MostRecentlyAccessedBabyId))
            {
                var familyBabies = await currentDataSource.GetBabies(currentUserProfile.FamilyId);
                var numberOfExistingBabies = familyBabies.Count();

                // If the family has no babies then continue to the create baby page
                if (numberOfExistingBabies == 0)
                {
                    // TODO - Create a 'create baby' page
                    firstPage = typeof(CreateBabyPage);
                }
                // If the family has an existing baby then set it as 'last used' for this user and continue to the main page
                else if (numberOfExistingBabies == 1)
                {
                    var baby = familyBabies.Single();
                    currentSettings.MostRecentlyAccessedBabyId = baby.Id;
                }
                // If the family has multiple babies then continue to the baby selector page
                else
                {
                    // TODO - Create a 'baby selctor' page
                    firstPage = typeof(MainPage);
                }
            }
